...
同网容器访问地址：`http://tradingagents-mcp:8080/mcp`
...
- TradingAgents 加入外部网络 `ai-stack`；OpenClaw 等 stack 也需挂载同一网络。
- 同网访问 URL：`http://tradingagents-mcp:8080/mcp`（推荐，无需暴露端口给 OpenClaw）。
- 若客户端在宿主机、仅做调试，可用 `http://<NAS局域网IP>:8080/mcp`（依赖 compose 中的 `8080:8080` 映射）。
